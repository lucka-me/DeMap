...
function generateElevationFillByOrder() {
    ...
    for (var i = 0; i < elevationIndex.length; i++) {
        if (i - currentStep * orderStep >= orderStep) {
            currentStep ++;
        }
...
function generateSVG() {
    if (pointList.length == 0 || delaunayList.length == 0 || polygonList.length == 0) {
        alert("Please generate points and delaunay first.");
        return;
    }
    // Get size
    var size = parseInt(document.getElementById("graphicSize").value);
    if (isNaN(size)) {
    ...
    downloadButton.className = "console-download";
